...
if($count==0)
{
    ...
    $errmsg_arr[] = 'Registration as an Admin is Successful You can now login';
    $errflag = true;
if($errflag) {
    $_SESSION['ERRMSG_ARR'] = $errmsg_arr;
    session_write_close();
...
elseif($count>0){
    mysql_query("INSERT INTO user (username, password)
VALUES ('$regusername', '".md5($_POST['regpassword'])."')");
$errmsg_arr[] = 'Registration is Successful You can now login';
$errflag = true;
    if($errflag) {
    $_SESSION['ERRMSG_ARR'] = $errmsg_arr;
    session_write_close();
...
if($adminpass == '') {
$errmsg_arr[] = 'Admin Password is missing';
$errflag = true;
}
